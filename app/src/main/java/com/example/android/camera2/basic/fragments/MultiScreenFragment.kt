...
import com.example.android.camera.utils.renderer.FilterGLRenderer
import com.example.android.camera.utils.renderer.GLRenderer
import com.example.android.camera2.basic.CameraActivity
import com.example.android.camera2.basic.R
import com.example.android.camera2basic.AspectFrameLayout
import com.example.android.camera2basic.CameraGLSurfaceView
import com.example.android.camera2basic.CameraSurfaceView
import com.gain.longexposure.renderer.MultiScreenRenderer
...
class MultiScreenFragment : Fragment() {
    ...
    private lateinit var mPreviewView: CameraSurfaceView
    private lateinit var mAspectFrameLayout: AspectFrameLayout

    val glRenderer: GLRenderer by lazy { GLRenderer() }
    val mLeftRenderer: FilterGLRenderer by lazy { FilterGLRenderer() }
    val mRightRenderer: FilterGLRenderer by lazy { FilterGLRenderer() }
    ...
    ): View? = inflater.inflate(R.layout.fragment_multi_screen, container, false)

    @SuppressLint("MissingPermission")
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        overlay = view.findViewById(R.id.overlay)
        mAspectFrameLayout = view.findViewById(R.id.continuousCapture_afl)
        ...
        mPreviewView.holder.addCallback(object :SurfaceHolder.Callback{
            ...
            override fun surfaceCreated(holder: SurfaceHolder?) {
                glRenderer.createGL(null, holder!!.surface)
        ...
        var leftView:SurfaceView = view.findViewById(R.id.view_left)
        var rightView:SurfaceView = view.findViewById(R.id.view_right)

        mLeftRenderer.start()
        mRightRenderer.start()

        leftView.holder.addCallback(object :SurfaceHolder.Callback{
            override fun surfaceChanged(holder: SurfaceHolder?, format: Int, width: Int, height: Int) {
                mLeftRenderer.setDataSize(Size(width, height))
            }
            ...
            override fun surfaceCreated(holder: SurfaceHolder?) {
                if(GLRenderer.SHARED_EGL_CONTEXT!=null) {
                    mLeftRenderer.createGL(GLRenderer.SHARED_EGL_CONTEXT, holder!!.surface)
                } else {
                ...
                mainHandler.postDelayed({
                    mLeftRenderer.render()
                }, 1500)
            }
        })

        rightView.holder.addCallback(object :SurfaceHolder.Callback{
            override fun surfaceChanged(holder: SurfaceHolder?, format: Int, width: Int, height: Int) {
                mRightRenderer.setDataSize(Size(width, height))
            }
            ...
            override fun surfaceCreated(holder: SurfaceHolder?) {
                if(GLRenderer.SHARED_EGL_CONTEXT!=null) {
                    mRightRenderer.createGL(GLRenderer.SHARED_EGL_CONTEXT, holder!!.surface)
                } else {
                    mainHandler.postDelayed({
                        mRightRenderer.createGL(GLRenderer.SHARED_EGL_CONTEXT, holder!!.surface)
                    }, 1000)
...
//        mAspectFrameLayout.setAspectRatio(previewSize)

        glRenderer.setDataSize(previewSize)

        val texture = glRenderer.surfaceTexture
...
//            imageBitmap.compress(Bitmap.CompressFormat.JPEG, 100, output)
    ...
    companion object {
        private val TAG = MultiScreenFragment::class.java.simpleName
